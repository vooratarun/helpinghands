...
<div class='container1 separator' style='width:100%;margin-left:30px;'></div>
						<center><U><h3 style="font-family:Adobe Gothic Std">COLLECTIONS</h3></U></center><BR>
<table class="table2" style='width:80%;margin-left:60px;'>
	...
	<tr class='broom_heading'>
		...
		<th style='font-weight:bold;text-align:center;color:#eee;'>POSTED</th>
		<th style='font-weight:bold;text-align:center;color:#eee;'>ACTION</th>
	</tr>
        </thead>
        <?php
        $note=mysql_query("SELECT * FROM `collections` WHERE visibility='1' ORDER BY sno DESC");
        while($note_fet=mysql_fetch_array($note))
        {
		$nt=$note_fet['sno'];
        ?>
	<tr  class='odd' id="n<?php echo $nt;?>"><td><?php echo $note_fet['sno'];?></td><td><?php echo $note_fet['subject'];?></td></td><td><?php echo $note_fet['fulldate'];?></td><td><a onclick="deletec('<?php echo $nt;?>')">Delete</a><?php } ?></td></tr>

...
struct p_info
{
	int extime,artime,qintime,qouttime,state,et;
};
void makequeue(struct p_info arr[n])//sorting with bubble sort
{
	...
	for(i=0;i<n;i++)
	{
		for(j=0;j<n-1;j++)
		{
			...
			{
				temp.extime=arr[j].extime;
				temp.artime=arr[j].artime;
				temp.et=arr[j].et;
				arr[j].extime=arr[j+1].extime;
				arr[j].artime=arr[j+1].artime;
				arr[j].et=arr[j+1].et;
				arr[j+1].extime=temp.extime;
				arr[j+1].artime=temp.artime;
				arr[j+1].et=temp.et;
			}
		}
	}
...
main()
{
	...
	makequeue(plist);							//calling Ghant Chart

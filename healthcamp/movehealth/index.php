<?php
session_start();
if(isset($_SESSION['healthcampid']))
{
?>
<html>
<head>
...
<form method='post'><button  type='submit' style="position:absolute;top:5%;left:90%;width:50px;height:30px;" name='logout'>LogOut</button></form>
...
<td>ID No:-</td><td><input  style="height:30px;"type='text' name='uid' id='uid' maxlength='7' value="<?php echo $_SESSION['healthcampid'];?>"
 readonly></td>
...
<input type='checkbox' name='dental' value='Dental' >Dental</br>
<input type='checkbox' name='ear' value='Ear'>Ear</td>
...
if(isset($_POST['submit']))
{
	...
	if(@$_POST['ear']!="")$dental="yes";

	$c=mysql_query("SELECT uid FROM health WHERE uid='$id'") or die(mysql_error());
	$cf=mysql_fetch_array($c);
	if($cf['uid']!=$id)
{
		if($eye=='yes' || $skin=='yes' || $dental=='yes' || $ear=='yes'){
			$q1=mysql_query("INSERT INTO `health`( `uid`, `eye`, `skin`, `dental`,`ear`) VALUES('$id','$eye','$skin','$dental','$ear')");
		if($q1==true)
...
{
	echo "
<center><button onclick='window.location.href='index.php'>Back</button></center>
<script>$('#form').hide(1000);</script>";
$q=mysql_num_rows(mysql_query("SELECT * from health"));
	if($q!=0)
	{
		...
		$q1=mysql_num_rows(mysql_query('select * from health where ear="yes"'));
		echo "<center><h4><font color=green>Students registered for ear ...</font>$q1</h4></center>";
...
unset($_SESSION['healthcampid']);
	echo "<script>window.location.href='http://10.11.2.92/help'</script>";}
